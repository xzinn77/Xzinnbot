...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Trippie\n' 
            + 'ORG: BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=5597984341374:+55 9798434-1374\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '.'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["5597984341374@s.whatsapp.net","5597984341374@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oie @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* sega as regras, se n vai mama os adm e os membro tudo, ainda vai levar ban 😉, fique a vontade bb`
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply(" n fala isso seu merdinha ")
                        .then(() => client.groupRemove(from, sender))
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI BADWORD 」*\nVocê foi chutado por falar asperamente, seu merda!`, text ,{quoted: mek})
                        }).catch(() => client.sendMessage(from, `Felizmente não é o administrador, se o administrador já o chutou!`, text , {quoted : mek}))
                } else {
                    return reply( "😛😐😛😐")
                }
            }
            ...
            case 'bitly':
               ...
               case 'help': 
                ...
                if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @558897187831 20`)
...
• Limit: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'coloca o codigo do pais seu merda', text, {quoted: mek})
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aq tua ft '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
				break 
           ...
           case 'demote':
				...
				case 'block':
					...
					client.sendMessage(from, `ok, dando block wa.me${body.slice(8)}@c.us`, text)
				break
				case 'unblock':
					client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, `dando desblock... wa.me/${body.slice(10)}`, text)
				break 
				case 'setppbot':
					if (!isOwner) return reply(ind.ownerb())
				    client.updatePresence(from, Presence.composing) 
